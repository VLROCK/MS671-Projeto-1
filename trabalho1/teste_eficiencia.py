...
def executar_predicao(image_file, model, class_names, device, score_threshold=0.5, iou_threshold=0.4):
    ...
    with torch.no_grad():
        ...
        keep_torch = torchvision.ops.nms(boxes, scores, iou_threshold) # Isso é função pronta, não use isso no seu código, a menos que queira comparar com a sua implementação apenas
        torchvision_time = (time.time() - start_time) * 1000
        # Aplica o nosso NMS Manual

        start_time = time.time()
        keep_manual = manual_nms(boxes, scores, classes, iou_threshold) # Aqui entra a função manual de NMS que devem implementar
        manual_time = (time.time() - start_time) * 1000

        print("\n--- Resultados da Comparação do NMS ---")
        print(f"Caixas de entrada totais antes do NMS: {len(boxes)}")
        print(f"Torchvision NMS:")
        print(f"  - Caixas mantidas: {len(keep_torch)}")
        print(f"  - Tempo de execução: {torchvision_time:.4f} ms")
        print(f"Manual NMS:")
        print(f"  - Caixas mantidas: {len(keep_manual)}")
        print(f"  - Tempo de execução: {manual_time:.4f} ms")

        keep_manual_tensor = torch.tensor(keep_manual, dtype=torch.long, device=boxes.device)
        set_tv = set(keep_torch.cpu().numpy())
        set_man = set(keep_manual_tensor.cpu().numpy())
        if set_tv == set_man:
             print("  - Exatidão: SUCESSO! Ambas as funções escolheram as mesmas caixas.")
        else:
             print("  - Exatidão: DIVERGÊNCIA. As funções escolheram caixas diferentes.")
        print("---------------------------------------\n")
...
try:
    ...
    torch.save(modelo.state_dict(), "weights/yolov3_convertido.pth")
except Exception as e:
    print(f"Aviso na conversão (talvez os pesos já existam ou o arquivo não foi encontrado): {e}")

lista_imagens = ["dog.jpg", "eagle.jpg", "giraffe.jpg", "horses.jpg", "person.jpg", "city_scene.jpg", "food.jpg", "surf.jpg", "wine.jpg", "motorbike.jpg"]
# 4. Inferência direta!
# Caso você já tenha salvo o .pth anteriormente, apenas carregue-o:
modelo.load_state_dict(torch.load("weights/yolov3_convertido.pth", map_location=device))
